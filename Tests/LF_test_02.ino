...
#define lmf 19
#define lmb 23
#define rms 4   // Right motor speed (PWM)
#define lms 15  // Left motor speed (PWM)
...
float c;
int left_motor_speed = 255, right_motor_speed = 255;
...
void Line_Follow() {
  while (1) {
    ...
    if (sensor[2] == 1 || sensor[3] == 1) {
      current_error = 3 - c; // Adjusted for 5 sensors (center at 3)
    ...
    // Right Turn
    if (sensor[4] == 1 && sensor[0] == 0) t = 'r';
    // Left Turn
    if (sensor[4] == 0 && sensor[0] == 1) t = 'l';
    ...
    if (sensor[0] == 1 && sensor[1] == 1 && sensor[2] == 1 && sensor[3] == 1 && sensor[4] == 1) {
      ...
      } else if ((sensor[0] + sensor[1] + sensor[2] + sensor[3] + sensor[4]) == 0) t = 'r';
    }
  }
}
